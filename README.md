...
2. 目标获取方面，提供了多重目标输入方式，单个目标，IP段，文件、API（目前就只写了zoomeye、百度域名采集、subDomainsBrute）、爬虫（目前完成了域名采集爬虫）。
...
## 支持目标类型
1. 单一URL
2. IP段 192.168.1-254，192.168.1.0/24，10.10.10.1-10.10.20.1
3. 文件
4. 域名采集爬虫 根据指定的初始域名进行域名采集
5. 搜索引擎爬虫 目前仅百度，根据关键字采集域名
6. zoomeye
7. subDomainsBrute 根据subDomainsBrute返回的结果，对得到的子域名，对应IP和IP的C段进行扫描
